...
class UTildeFunction:
    ...
    def gradient(self, x):
        ...
        return np.array([-np.pi * np.sin(np.pi * x[1]) * np.sin(np.pi * x[0]),
                         np.pi * np.cos(np.pi * x[0]) * np.cos(np.pi * x[1])]).T
    ...
    def laplacian(self, x):
        ...
        return -np.pi ** 2 * np.sin(np.pi * x[1]) * np.cos(np.pi * x[0]) - np.pi ** 2 * np.cos(np.pi * x[0]) * np.sin(
            np.pi * x[1])
